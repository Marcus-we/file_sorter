...
def extract_common_keywords_from_text_cluster(cluster_files: List[Dict[str, Any]]) -> List[str]:
    ...
    common_keywords = [word for word,
                       count in keyword_counter.most_common(5) if count > 1]
...
def generate_text_category_name(cluster_files: List[Dict[str, Any]]) -> str:
    ...
    if keywords:
        ...
        if extensions and extensions[0] in ['.doc', '.docx', '.pdf', '.txt']:
            name += " Documents"
        elif extensions and extensions[0] in ['.md', '.rst']:
            name += " Documentation"
...
def generate_category_names(clusters: Dict[int, List[Dict[str, Any]]]) -> Dict[int, str]:
    ...
    for cluster_id, cluster_files in clusters.items():
        ...
        text_count = sum(1 for f in cluster_files if f.get(
            'content_type') == 'text')
        image_count = sum(1 for f in cluster_files if f.get(
            'content_type') == 'image')
        ...
        if text_count >= image_count and text_count >= other_count:
            category_names[cluster_id] = generate_text_category_name(
                cluster_files)
        elif image_count >= text_count and image_count >= other_count:
            category_names[cluster_id] = generate_image_category_name(
                cluster_files)
        else:
            category_names[cluster_id] = generate_other_category_name(
                cluster_files)
